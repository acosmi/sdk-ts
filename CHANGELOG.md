...
- 新增导出 `isStreamError(err): err is StreamError` —— `name === 'StreamError'` ∧ `errorCode` 是字符串的**结构**判据。消费方在 worker / `vm` 上下文里，或 SDK 被打进第二份 bundle 时，`instanceof StreamError` 会静默为 `false`，症状与「这段分类代码压根没写」逐字相同。`HTTPError` 也带可选 `errorCode`，靠 `name` 分开。
- 新增 `ChatStreamOptions`，作为 `chatStream` / `chatMessagesStream` 的**末位**可选实参。`requestTimeoutMs` 声明这一次流的总时长安全网（从开始迭代算起，与调用方 `signal` 先到者生效，`break` 提前退出也会释放计时器）；`onResponseHeaders` 在拿到响应之后、判断 `resp.ok` **之前**交出整份响应头，用于读网关随头下发的流控提示（如 `X-Acosmi-Stream-Keepalive`，秒）—— 那个数只在头里，等流内事件时已经晚了，而最需要它的恰恰是一个事件都不来的流。与既有两个流式回调同构：旁路信号，回调抛错被吞掉且不中断流；401 刷新重试后由真正开始流的那次响应触发。
- `FileTokenStore.withLock` 不再把 Windows 的 delete-pending 当成致命错误。NT 在 `unlink` 之后、最后一个句柄关闭之前把文件置为 `STATUS_DELETE_PENDING`，这期间 `open(lockPath, 'wx')` 抛的是 `EPERM`（实测原文 `EPERM: operation not permitted, open '...tokens.json.lock'`；`EBUSY` 是同机理的第二种表现）而**不是** `EEXIST`；判据只认 `EEXIST`，于是同一把锁上的一次正常交接被 `throw e` 报成永久失败 —— 同进程 5 个并发 `withLock` 就能稳定复现，受害者是 refresh token rotation 那段临界区。现在「锁被占」的判据是 `EEXIST` ∪（`win32` 且 code ∈ {`EPERM`, `EBUSY`}），命中后者时**跳过 stale 检测**直接退避重试（delete-pending 的文件 `stat` 要么失败要么给出前一持有者的旧 mtime，两种都会把「正在消失的锁」误判成「崩溃残留」，而 `stat` 失败那条腿是 `continue`、不过 `acquireTimeoutMs` 检查，会退化成不退出的忙循环），重试仍受 `acquireTimeoutMs` 上界约束。`EACCES` **不在**这个集合里：它在 Windows 上更常见的产地是锁目录只读 / 被策略拒绝这类真配置错误，收进来等于把「立刻报 `EACCES`」拖成「30 秒后报 acquire timeout」。**非 Windows 平台控制流逐字节不变** —— POSIX 的 `EPERM` 同样是真权限错误，一律不收。
- **零破坏性变更**：前三项全部是追加式（新可选字段 / 新导出 / 末位新可选实参），2.19.4 的调用方式一字不改仍然成立。`requestTimeoutMs` 缺席或非有限正数时**不武装任何计时器**，与 2.19.4 逐字一致 —— 流式链路历来只受调用方 `signal` 约束（`CHAT_REQUEST_TIMEOUT_MS` 的注释此前误把两个流式方法也算了进去，本版同步勘误：那两条路径从来没有武装过它）。给历来无上限的流凭空补一道默认墙钟，第一个被杀的就是健康的长任务。

## 2.19.4

- Stop dropping the OpenAI-line streaming usage tail frame. With `stream_options.include_usage: true` the upstream sends content frames, then the `finish_reason` frame, then `{"choices":[],"usage":{...}}`, then `data: [DONE]`. The stream converter discarded every frame without `choices`, and it had already emitted `message_delta` (without usage) and `message_stop` on the `finish_reason` frame, so no OpenAI-line turn streamed through `chatMessagesStream` ever carried usage and consumers counted it as zero. The converter now records the usage object of any frame, with or without `choices` (later frames override earlier ones), and puts it in the `message_delta` that immediately precedes the single `message_stop`.
- Content blocks are still closed on the `finish_reason` frame, but when no usage has been seen yet, `message_delta` and `message_stop` wait for the first of: a frame carrying usage, `data: [DONE]`, or the end of the body. The new `OpenAIStreamConverter.flush()` emits that deferred close at end of stream; `chatMessagesStream` calls it when the body ends without `[DONE]`, and code that drives the converter directly should do the same. `flush()` never fabricates a close for a stream that never delivered `finish_reason`, and no path emits `message_stop` twice.
- The streaming usage mapping is exactly the non-streaming one: `prompt_tokens` to `input_tokens` and `completion_tokens` to `output_tokens`. Cached-token and reasoning-token details are neither mapped nor netted out; normalizing usage semantics belongs to the gateway. A count that is missing or not a finite number is omitted rather than written as 0. `OpenAIStreamChunk.usage` is now typed as `OpenAIStreamUsage | null`, with optional counts and the detail objects.
- Fix the `@acosmi/sdk-ts/adapters/openai` and `@acosmi/sdk-ts/adapters/anthropic` subpath entries, which threw as soon as they were loaded: `OpenAIAdapter is not a constructor` / `AnthropicAdapter is not a constructor` under Node ESM and CJS, `undefined is not a constructor` under Bun. Both adapter implementations took `ProviderFormat` from the adapters barrel, and the barrel constructs both adapters at module top level. With a subpath as the entry, the adapter module was evaluated first and pulled in the barrel, whose top-level `new` then ran before the adapter class had been defined. The root entry `@acosmi/sdk-ts` loads the barrel first and was not affected; the published 2.19.3 package fails the same way. `ProviderFormat` and `ProviderAdapter` now live in a leaf module that imports no adapter implementation, and every entry exports the same names as before. `test:pack` now also loads every `exports` subpath that declares an `import` or `require` condition, each in a fresh Node process, with `import()` and `require()` respectively; before, it only type-checked from the consumer side and never loaded any entry at runtime.
- Tolerate a stream `choice` that carries no `delta`. Some compatible implementations send `{"choices":[{"index":0}]}` to say "channel 0 is still there" on a frame with no increment; the converter read `choice.delta.reasoning_content` straight away, and the resulting `TypeError` tore through the whole `for-await` chain and failed the turn. Such a choice is now treated as an empty delta: zero events, no error, the stream keeps running, and `finish_reason` / `usage` on the same frame are still handled. `OpenAIStreamChoice.delta` is now optional, matching what the wire actually sends.
- Stop throwing on a non-streaming response that carries no `choices`. Both `parseResponse` and `parseOpenAIResponseToAnthropic` read `oai.choices.length` without a guard, so the whole response died - including the `id`, `model` and `usage` that had in fact arrived. Missing `choices` is now treated as an empty array on both paths: no content blocks, no error, `stop_reason` left empty rather than invented, and every other field converted as before. `OpenAIChatResponse.choices` is now optional. This is the fourth known site of one recurring mistake - a wire type declaring a key required that the wire genuinely omits - after `OpenAIStreamChunk.choices`, `OpenAIStreamChoice.delta` and `OpenAIChatResponse.usage`.
- Stop throwing on a non-streaming response that carries no `usage`. Both `parseResponse` and `parseOpenAIResponseToAnthropic` read `oai.usage.prompt_tokens` without a guard, so a response whose content, `finish_reason` and `tool_calls` were all perfectly usable was killed outright just because the upstream does not meter. Missing usage now counts as 0 on both paths and everything else converts as before; `OpenAIChatResponse.usage` is now optional.
- Fix block indexes when text arrives before `reasoning_content`. The thinking branch neither closed an open text block nor advanced the block index, so both `content_block_start` events landed on index 0 while the `textStarted / else if thinking` close only ever reached the text block - the thinking block was never closed. The thinking branch now mirrors the text and `tool_calls` branches and closes an open text block first, so at most one non-tool block is open at a time. This is the other half of the invariant the `thinking` then `tool_calls` fix established.
- Add `WindowLimitStatus.fiveHourSoftContinuing?: boolean`, the gateway's server-synthesized flag meaning the window is used up but the user's "continue" switch is letting requests through. New gateways always send it and older gateways omit it, so absence does not mean `false`. Type only.
- Add `WindowLimitStatus.inflightHeldCredits?: number | null`, the micro-Credits in that window currently held by in-progress requests and not yet settled. `usedCredits` counts settled usage only, while admission counts settled usage plus in-flight holds; this field is the difference. Absent or `null` means unknown (an older gateway, or the lookup failed), not 0; `0` means nothing is in flight. Type only.
- Add `QuotaSummary.subscriptionPool?: QuotaSummarySubscriptionPool` (`quota`, `used`, `remaining`, optional `expiresAt`, `unit`), derived by the gateway from alive COMMERCIAL buckets. Amounts are in micro-Credits (divide by 1000 for Credits). The field is absent on older gateways and when there is no alive paid bucket; do not read absence as 0. Type only.
