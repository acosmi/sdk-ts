...
- **当前版本：`2.19.5`**（流内重试提示透出 + 流式可选项，2026-09-19）。
...
### 流式可选项 `ChatStreamOptions`（v2.19.5+）

`chatStream` / `chatMessagesStream` 的**末位**可选实参。两项都是追加语义，不传时行为与既往逐字一致。
...
const stream = client.chatMessagesStream(
  ...
  onGatewayRequestID,
  {
    requestTimeoutMs: 60 * 60 * 1000,        // ← 安全网，不是主判据
    onResponseHeaders: h => {
      const keepalive = h.get('X-Acosmi-Stream-Keepalive'); // 秒；缺失为 null
      if (keepalive) armIdleWatchdog(Number(keepalive));
    },
  },
);
```

- **`requestTimeoutMs`** —— 这一次流的**总时长**预算，从开始迭代算到流结束；与 `signal` 先到者生效，调用方 `break` 提前退出也会释放计时器。**缺席或非有限正数 = 不武装任何计时器**（流式链路历来只受调用方 `signal` 约束；`CHAT_REQUEST_TIMEOUT_MS` 只作用于非流式的 `chat` / `chatMessages`）。杀一条流的判据应当是**失去活性证据**（见上面的 `onUpstreamActivity`）而不是「活得太久」—— 这一条的职责只是给「看门狗本身坏掉」兜底，因此应当显著长于活性预算。
- **`onResponseHeaders`** —— 在拿到响应之后、判断 `resp.ok` **之前**触发，因此 HTTP 错误响应上同样触发；401 刷新重试后由真正开始流的那次响应触发。网关的流控提示只存在于响应头里，等流内事件已经晚了，而最需要它的恰恰是一个事件都不来的流。与其它两个流式回调同构：旁路信号，回调抛错被吞掉且不中断流。
...
| `StreamError`        | gateway `managed_model_stream_failed` 事件解析（含 `code` / `stage` / `retryable`；v2.19.5 起另带 `serverRetryable` = 帧上 `retryable` 原值、`retryAfterSecs` = 建议等待秒数，缺席均为 `undefined`。跨 realm 用 `isStreamError()` 判，别用 `instanceof`） |
...
| 版本 | 状态 | 概要 |
| --- | --- | --- |
| 2.19.6 | **当前版本** | **订阅池加油包份额（2026-09-20）**。`QuotaSummarySubscriptionPool` 加性新增 `boosterRemaining?` / `boosterCount?` / `boosterNextExpiresAt?`：网关在既有那一次 alive 遍历里，对 alive 的 COMMERCIAL 桶中 `sourceType === 'booster_pack'` 的行求和（微 Credits，÷1000 = Credits）、计数（一行 = 一个加油包）、并在这些 alive **限期**桶里选举最早到期时刻。**三者缺席 = 老网关或此刻没有加油包，都不可读成 0**（网关一行都认不出时三个键一起不下发，绝不写 0）；`boosterNextExpiresAt` 另可因「加油包全是永久桶」而单独缺席，此时前两个字段仍在场。加油包是「加总额度」的产品动作，与只清 5 小时 / 7 天窗口、对额度池零交互的重置卡是两笔账。纯类型面追加，无运行时改动。 |
| 2.19.5 | 稳定版 | **流内重试提示透出 + 流式可选项（2026-09-19）**。`StreamError` 加性新增 `serverRetryable`（帧上 `retryable` **原值**，不与 `requestDisposition` 合取 —— 合取会把「服务端明说可重试」与「服务端没说」压成同一个 `false`）与 `retryAfterSecs`（只收有限非负数，非法值一律 `undefined`）；加性导出结构判据 `isStreamError()`（worker / `vm` / 二次打包下 `instanceof` 静默为假）；`chatStream` / `chatMessagesStream` 新增**末位**可选实参 `ChatStreamOptions`（`requestTimeoutMs` 总时长安全网、`onResponseHeaders` 在 `resp.ok` 判断前交出响应头，用于读 `X-Acosmi-Stream-Keepalive` 等流控提示）。全部追加式；不传 `requestTimeoutMs` 时**不武装任何计时器**，流式链路行为与 2.19.4 逐字一致。另修 `FileTokenStore.withLock` 在 Windows delete-pending 窗口把 `EPERM` / `EBUSY` 当成致命错误（NT 在 `unlink` 之后、最后一个句柄关闭之前抛的不是 `EEXIST`，于是同一把锁上的一次正常交接被报成永久失败）；这两个码现在按「锁被占」退避重试并跳过 stale 检测，`EACCES` 刻意不收，非 Windows 平台控制流逐字节不变。 |
| 2.19.4 | 稳定版 | **OpenAI 线流式 usage 尾帧回收 + 子路径入口修复 + 一批 wire 容错**。`stream_options.include_usage` 下上游在 `finish_reason` 帧之后才发 `{"choices":[],"usage":{...}}`，转换器此前丢弃一切不带 `choices` 的帧、且已在 `finish_reason` 帧上发出不带 usage 的 `message_delta` 与 `message_stop`，于是 OpenAI 线经 `chatMessagesStream` 的回合从来没有带过 usage、消费方一律记成 0；现在任何帧上的 usage 都被记录（后帧覆盖前帧）并写进 `message_stop` 前紧邻的那条 `message_delta`。内容块仍在 `finish_reason` 帧关闭，但尚未见到 usage 时 `message_delta` / `message_stop` 会等「带 usage 的帧 / `data: [DONE]` / body 结束」三者中最先到的一个，新增 `OpenAIStreamConverter.flush()` 在流末补发这次延迟收口（绝不为从未给过 `finish_reason` 的流伪造收口，也没有任何路径会发两次 `message_stop`）。流式 usage 映射与非流式逐字相同（`prompt_tokens`→`input_tokens`、`completion_tokens`→`output_tokens`），缓存 token 与 reasoning token 明细既不映射也不冲抵，缺失或非有限数一律省略而不是写 0。修复 `@acosmi/sdk-ts/adapters/openai` 与 `@acosmi/sdk-ts/adapters/anthropic` 两个子路径入口**加载即抛**（Node ESM / CJS 下 `OpenAIAdapter is not a constructor`、Bun 下 `undefined is not a constructor`）：`ProviderFormat` / `ProviderAdapter` 迁进不 import 任何 adapter 实现的叶子模块，各入口导出名一字不变；`test:pack` 现在会在各自全新的 Node 进程里对每个声明了 `import` 或 `require` 条件的 `exports` 子路径真加载一次。另容忍不带 `delta` 的流 `choice`、不带 `choices` 的非流式响应、不带 `usage` 的非流式响应（分别按空 delta / 空数组 / 0 处理，不再抛错，其余字段照常转换）；修正 text 先于 `reasoning_content` 到达时两个 `content_block_start` 都落在 index 0 的块序错乱。类型面加性新增 `WindowLimitStatus.fiveHourSoftContinuing?`、`WindowLimitStatus.inflightHeldCredits?`、`QuotaSummary.subscriptionPool?` —— 三者缺席都表示「旧网关没播报」，不得读成 `false` / `0`。 |
| 2.19.3 | 稳定版 | **OpenAI 线网关流失败帧不再把自己撞崩**。网关用 `event: failed` 发结构化错误帧，而 `chatMessagesStream` 的 OpenAI 分支捕获了 SSE 事件名却从不读它，错误帧于是被送进 OpenAI chunk 转换器、死在 `chunk.choices.length` 上 —— 真实上游原因被一个无关的 TypeError 顶掉。该分支现在把 `failed` / `error` 帧交给既有的 `parseStreamError`，与 `chatStreamWithUsage` 早已在做的完全一致；`OpenAIStreamChunk.choices` 同版改为可选（类型声明必填而 wire 确实会省，正是那个 TypeError 的来处）。Anthropic 线零行为变化，请求构造零变化。 |
| 2.19.2 | 稳定版 | **思考档位透传补全**。目录支持的 low / medium / xhigh / max 四档在两个 chat adapter 上原样保留；显式「关闭思考」的请求被真正投递给 OpenAI 兼容网关；2.19.1 的凭据与请求 ID 行为保持不变。 |
| 2.19.1 | 稳定版 | **认证生命周期与上游错误归属修复（2026-09-08）**。新增显式 `credentialMode: 'versioned'`，通过调用方提供的 `VersionedCredentialStore` 原子读写凭据，多进程同时刷新共用同一份持久状态、刷新结果不确定时要求重新登录；新增 `credentialRequestOwner` 把请求绑定到构造时的存储实例、登录会话与已验证身份（已退出或被替换的客户端不能借用另一个账号的凭据，WebSocket 票据与晚到事件遵守同一归属）；新增 `logoutCredential(signal?, expected?)`，本地提交结果与远端吊销结果分开；新增显式 `credentialMode: 'external'` 与 `accessTokenProvider`。HTTP 401 只有在完整错误合同明确归属用户访问凭据失效、且请求未被接收时才触发一次刷新重试，提供商拒绝 / 已接收请求 / 缺失合同的错误不再被误当成登录过期。默认仍为 `legacy` 模式。 |
| 2.19.0 | 稳定版 | **网关消费请求 ID 透出（2026-09-01）**。网关把 `consumeRequestID`（即 `managed_model_usage_logs.request_id`）放进 `X-Acosmi-Request-Id` 响应头；加性导出 `GATEWAY_REQUEST_ID_HEADER` 与 `GatewayRequestIDCallback`，`chatMessagesStream` / `chatStream` 新增第 5 个可选实参、`chatMessages` / `chat` 新增第 4 个可选实参。响应头在首字节之前到达，因此覆盖流中段中断 / 零事件 / HTTP 错误全部形态。旧网关下回调零触发、绝不合成占位值，流照常出。不传回调时行为逐字节不变。 |
