...
> **为什么不用 tag push**：v2.14.0 发版时，**连续 5 次**创建 tag（REST 建 ref ×1、
> 浅仓 `git push` ×3、完整 clone `git push` ×1）**没有产生任何 workflow run**。
> 已逐条排除：tag 存在且指向正确 commit、workflow 状态 `active`、Actions permissions
> `enabled`+`allowed_actions: all`、公开仓 Actions 分钟数无限、rulesets 为空、
> 被 tag 的 commit 上 `release.yml` 存在且 `on: push: tags: ['v*']` 匹配、commit
> message 不含 `[skip ci]`。仓库事件流里连 `CreateEvent` 都没有（同期三次 tag
> 删除的 `DeleteEvent` 都在），而 push-to-main 触发的 CodeQL 全部正常。同一机制
> 9 小时前发 v2.13.0 时还工作正常 —— **成因未定位**。`release.yml` 因此补了
> `workflow_dispatch`（2026-08-02），它是当前唯一被实测验证可用的入口。
> tag push 触发保留在 `on:` 里，哪天自己恢复了也不冲突。

**两个真实存在、但本次已被排除为成因的 GitHub 语义**（仍值得知道，别再花时间重排）：

- `[skip ci]` 对 **tag push 同样生效**，而 subtree split **原样保留原始 commit
  message** —— monorepo 里带 `[skip ci]` 的发版 commit 会把标记一路带进公开仓。
  发版 commit 建议不带该标记。（本次修掉后 tag push 仍不触发，故非本次成因。）
- REST 建 ref（`gh api .../git/refs -X POST`）只产生 `create` 事件、不产生 `push`
  事件，因此对 `on: push: tags` 无效。（本次改用真 `git push` 后仍不触发，同上。）
