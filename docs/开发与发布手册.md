...
`acosmi-sdk-ts` 是 Acosmi 模型网关的 TypeScript SDK，从 `acosmi-sdk-go` 全量端口而来，**自 v1.0.0 起与 Go SDK 解耦独立演进**。

| 项目     | 值 |
| -------- | -- |
| 端口源   | `acosmi-sdk-go` v0.19.0（bug-for-bug 一字不差） |
| 当前版本 | 1.0.0（稳定测试版） |
| 版本策略 | **不再对齐 Go SDK**，TS 独立 semver |
...
**自 v1.0.0 起独立 semver，不再对齐 Go SDK**。

- patch（1.0.x）：bug fix、文档、内部 refactor
- minor（1.x.0）：新增 API、新增模块、新参数选项
- major（x.0.0）：wire-format 不兼容变更、API 移除、TokenStore 接口变化

Go SDK 升级时：参考改动列表选择性同步（见 §11），不强对齐。

## 11. Go SDK 同步流程

每次 Go SDK 升级（不强对齐，按需）：
...
5. 升 TS version（独立 semver，不对齐 Go）
6. 走发布流程 §9.2
