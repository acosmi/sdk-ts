...
- 2026-05-01 07:51:35Z — `npm publish @acosmi/sdk-ts@1.0.0`（首发）
- 同日 — 下游 crabcode（Anthropic 兼容格式消费方）反馈 P0 双层 broken
- 同日 — 修复并发 `1.0.1` + `npm deprecate @acosmi/sdk-ts@1.0.0`
...
- 修复 commits：`f4972d3`（C1 Layer 1）→ `1597d61`（C2 Layer 2a 6 处 + tsconfig paths）→ `47e7307`（C3 Layer 2b 3 处）→ `9f94561`（C4 Layer 3 烟测）→ `b8feb5b`（C5 版本同步 1.0.0→1.0.1）→ C6（本节 + CHANGELOG + README 更新历史）
