...
`@acosmi/sdk-ts` 是 Acosmi 模型网关与 SDK-facing Agent Run Gateway 的 TypeScript SDK，从 `acosmi-sdk-go` 全量端口而来，**自 v1.0.0 起 Go SDK + TS SDK 联动稳定测试版基线，patch 号独立**。

| 项目     | 值 |
| -------- | -- |
| 端口源   | `acosmi-sdk-go` v1.0.0（与 Go SDK 联动稳定测试版） |
| 当前版本 | 1.2.0（InputModality + 桌面视觉理解 sidecar 契约） |
...
│   └── ...
├── test/                      # vitest 单元测试 (9 文件, 79 cases)
...
- 同日 — 主仓 `chat-acosmi` 7 commit 修复落地（C0~C6: 4d53585 → 885c08a）
- 同日 — sdk 仓 `sdk-ts-clean-tmp` 同步 + amend reset-author + force push（commit `e60a88e → 0d8c0a9` 单 commit history 重写）
...
**主仓 chat-acosmi 修复 commits** (开发原始历史，含 Co-Authored-By Claude)：
...
**sdk 仓 sdk-ts-clean-tmp 单 commit 发布历史** (公开仓干净 history，作者 acosmi-fushihua)：
